...
设置批量生成的范围 | scope | `npm run build-all scope path/to/foo` | 自动查找`src/path/to/foo`下的所有的项目, 依次自动编译所有；同`build`命令, `scope`的值也可以包含多个, 以`,`间隔。
选择部署方式 | key值 | `npm run build foo test` | `pro`是`config.deployEnvType`中定义的key值, 默认是`test\|pre\|pro`.
上传代码到ftp | u | `npm run build foo test u`或`npm start u foo` | `foo`在test模式编译后, 立即上传; 或指定上传的项目。(上传目录在`config.localAssetPath`设定。)
